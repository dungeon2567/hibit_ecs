...
/* ============================================================
   vec3 — bit-exact known values
   ============================================================ */

static void test_vec3_basic(void) {
    vec3_t v = vec3_make(fixed_from_int(1), fixed_from_int(2), fixed_from_int(3));
    EXPECT(v.x == fixed_from_int(1), "vec3_make x");
    EXPECT(v.y == fixed_from_int(2), "vec3_make y");
    EXPECT(v.z == fixed_from_int(3), "vec3_make z");
    EXPECT(v._pad == 0,              "vec3_make pad=0");

    vec3_t z = vec3_zero();
    EXPECT(z.x == 0 && z.y == 0 && z.z == 0 && z._pad == 0, "vec3_zero");

    fixed_t buf[3] = { fixed_from_int(7), fixed_from_int(-8), FIXED_HALF };
    vec3_t l = vec3_load(buf);
    EXPECT(l.x == buf[0] && l.y == buf[1] && l.z == buf[2], "vec3_load");
    fixed_t out[3] = {0};
    vec3_store(out, l);
    EXPECT(out[0] == buf[0] && out[1] == buf[1] && out[2] == buf[2], "vec3_store");
}

static void test_vec3_arith_known(void) {
    vec3_t a = vec3_make(fixed_from_int(1), fixed_from_int(2), fixed_from_int(3));
    vec3_t b = vec3_make(fixed_from_int(4), fixed_from_int(5), fixed_from_int(6));

    vec3_t s = vec3_add(a, b);
    EXPECT(s.x == fixed_from_int(5) && s.y == fixed_from_int(7) && s.z == fixed_from_int(9),
           "vec3_add (1,2,3)+(4,5,6)");
    EXPECT(s._pad == 0, "vec3_add pad");

    vec3_t d = vec3_sub(b, a);
    EXPECT(d.x == fixed_from_int(3) && d.y == fixed_from_int(3) && d.z == fixed_from_int(3),
           "vec3_sub (4,5,6)-(1,2,3)");

    vec3_t n = vec3_neg(a);
    EXPECT(n.x == fixed_from_int(-1) && n.y == fixed_from_int(-2) && n.z == fixed_from_int(-3),
           "vec3_neg");
    EXPECT(n._pad == 0, "vec3_neg pad");

    vec3_t k = vec3_scale(a, fixed_from_int(2));
    EXPECT(k.x == fixed_from_int(2) && k.y == fixed_from_int(4) && k.z == fixed_from_int(6),
           "vec3_scale by 2");
    EXPECT(k._pad == 0, "vec3_scale pad");

    vec3_t h = vec3_hadamard(a, b);
...
static void test_vec3_dot_cross_known(void) {
    ...
    /* 1*4 + 2*5 + 3*6 = 32 */
    EXPECT(vec3_dot(a, b) == fixed_from_int(32), "vec3_dot = 32");

    vec3_t c = vec3_cross(a, b);
    EXPECT(c.x == fixed_from_int(-3) && c.y == fixed_from_int(6) && c.z == fixed_from_int(-3),
           "vec3_cross (1,2,3)x(4,5,6)");

    vec3_t ex = vec3_make(FIXED_ONE, 0, 0);
    vec3_t ey = vec3_make(0, FIXED_ONE, 0);
    vec3_t ez = vec3_make(0, 0, FIXED_ONE);
    vec3_t cz = vec3_cross(ex, ey);
    EXPECT(cz.x == ez.x && cz.y == ez.y && cz.z == ez.z, "x_hat x y_hat = z_hat");

    vec3_t self = vec3_cross(a, a);
    EXPECT(self.x == 0 && self.y == 0 && self.z == 0, "a x a = 0");

    EXPECT(vec3_dot(ex, ey) == 0, "x_hat . y_hat = 0");
}

static void test_vec3_length_normalize_known(void) {
    /* (3,4,0): 9+16=25 exactly; sqrt is exact. */
    vec3_t v = vec3_make(fixed_from_int(3), fixed_from_int(4), 0);
    EXPECT(vec3_length_sq(v) == fixed_from_int(25), "len_sq (3,4,0) = 25");
    EXPECT(vec3_length(v)    == fixed_from_int(5),  "len (3,4,0) = 5");
    ...
    vec3_t ex = vec3_make(FIXED_ONE, 0, 0);
    EXPECT(vec3_length(ex) == FIXED_ONE, "len (1,0,0) = 1");
}

static void test_vec3_lerp_known(void) {
    vec3_t a = vec3_make(0, 0, 0);
    vec3_t b = vec3_make(fixed_from_int(10), fixed_from_int(10), fixed_from_int(10));

    vec3_t m = vec3_lerp(a, b, FIXED_HALF);
    EXPECT(m.x == fixed_from_int(5) && m.y == fixed_from_int(5) && m.z == fixed_from_int(5),
           "lerp t=.5 -> midpoint");

    vec3_t z = vec3_lerp(a, b, 0);
    EXPECT(z.x == 0 && z.y == 0 && z.z == 0, "lerp t=0 -> a");

    vec3_t o = vec3_lerp(a, b, FIXED_ONE);
...
static void test_quat_axis_angle_rotate_known(void) {
    ...
    vec3_t y_axis = vec3_make(0, FIXED_ONE, 0);
    quat_t qy180 = quat_from_axis_angle(y_axis, FIXED_PI);

    EXPECT(qy180.x == 0     && qy180.y == 65830 &&
           qy180.z == 0     && qy180.w == 0,     "qy180 pinned");

    vec3_t v2 = vec3_make(FIXED_ONE, 0, 0);
    vec3_t r2 = quat_rotate_vec3(qy180, v2);
    EXPECT(r2.x == -66715 && r2.y == 0 && r2.z == 0, "rotate (1,0,0) by qy180 pinned");

    /* Identity rotation is exact. */
    vec3_t v3 = vec3_make(fixed_from_int(7), fixed_from_int(-3), FIXED_HALF);
    vec3_t r3 = quat_rotate_vec3(quat_identity(), v3);
...
static int test_math_all(void) {
    int before = g_failed;
    printf("=== math.h tests ===\n\n");
    test_fixed_sqrt_known();
    test_fixed_rsqrt_known();
    test_fixed_sincos_known();
    test_vec3_basic();
    test_vec3_arith_known();
    test_vec3_dot_cross_known();
    test_vec3_length_normalize_known();
    test_vec3_lerp_known();
    test_quat_basic();
    test_quat_mul_known();
    test_quat_axis_angle_rotate_known();
    test_quat_normalize_known();
    test_quat_nlerp_known();
