...
typedef __m128i fixed_4_t;

static inline fixed_4_t fixed4_zero(void)                            { return _mm_setzero_si128(); }
static inline fixed_4_t fixed4_set1(fixed_t x)                       { return _mm_set1_epi32(x); }
static inline fixed_4_t fixed4_set(fixed_t a, fixed_t b, fixed_t c, fixed_t d) { return _mm_set_epi32(d, c, b, a); }
static inline fixed_4_t fixed4_load(const fixed_t* p)                { return _mm_loadu_si128((const __m128i*)p); }
static inline fixed_4_t fixed4_load_aligned(const fixed_t* p)        { return _mm_load_si128((const __m128i*)p); }
static inline void      fixed4_store(fixed_t* p, fixed_4_t v)        { _mm_storeu_si128((__m128i*)p, v); }
static inline void      fixed4_store_aligned(fixed_t* p, fixed_4_t v){ _mm_store_si128((__m128i*)p, v); }

static inline fixed_4_t fixed4_add_impl_(fixed_4_t a, fixed_4_t b) { return _mm_add_epi32(a, b); }
static inline fixed_4_t fixed4_sub_impl_(fixed_4_t a, fixed_4_t b) { return _mm_sub_epi32(a, b); }
static inline fixed_4_t fixed4_neg_impl_(fixed_4_t a)              { return _mm_sub_epi32(_mm_setzero_si128(), a); }
...
static inline fixed_4_t fixed4_mul_impl_(fixed_4_t a, fixed_4_t b) {
    __m128i ae = _mm_mul_epi32(a, b);                                /* lanes 0,2 -> int64 */
    __m128i ash = _mm_shuffle_epi32(a, _MM_SHUFFLE(3, 3, 1, 1));
    __m128i bsh = _mm_shuffle_epi32(b, _MM_SHUFFLE(3, 3, 1, 1));
    ...
    return _mm_blend_epi16(ae, ao_shifted, 0xCC);
}

static inline fixed_4_t fixed4_div_impl_(fixed_4_t a, fixed_4_t b) {
    int32_t ax[4], bx[4], rx[4];
    _mm_storeu_si128((__m128i*)ax, a);
    _mm_storeu_si128((__m128i*)bx, b);
    for (int i = 0; i < 4; ++i) rx[i] = fixed_div(ax[i], bx[i]);
    return _mm_loadu_si128((const __m128i*)rx);
}

/* Compare ops return a 4-bit mask: bit i set iff lane i true. */
static inline uint8_t fixed4_eq_impl_(fixed_4_t a, fixed_4_t b) { return (uint8_t)_mm_movemask_ps(_mm_castsi128_ps(_mm_cmpeq_epi32(a, b))); }
static inline uint8_t fixed4_gt_impl_(fixed_4_t a, fixed_4_t b) { return (uint8_t)_mm_movemask_ps(_mm_castsi128_ps(_mm_cmpgt_epi32(a, b))); }
static inline uint8_t fixed4_lt_impl_(fixed_4_t a, fixed_4_t b) { return (uint8_t)_mm_movemask_ps(_mm_castsi128_ps(_mm_cmpgt_epi32(b, a))); }
static inline uint8_t fixed4_ge_impl_(fixed_4_t a, fixed_4_t b) { return (uint8_t)((~_mm_movemask_ps(_mm_castsi128_ps(_mm_cmpgt_epi32(b, a)))) & 0xF); }
static inline uint8_t fixed4_le_impl_(fixed_4_t a, fixed_4_t b) { return (uint8_t)((~_mm_movemask_ps(_mm_castsi128_ps(_mm_cmpgt_epi32(a, b)))) & 0xF); }

#elif FIXED_BACKEND_NEON

typedef int32x4_t fixed_4_t;

static inline fixed_4_t fixed4_zero(void)                            { return vdupq_n_s32(0); }
static inline fixed_4_t fixed4_set1(fixed_t x)                       { return vdupq_n_s32(x); }
static inline fixed_4_t fixed4_set(fixed_t a, fixed_t b, fixed_t c, fixed_t d) {
    int32_t v[4] = { a, b, c, d };
    return vld1q_s32(v);
}
static inline fixed_4_t fixed4_load(const fixed_t* p)                { return vld1q_s32(p); }
static inline fixed_4_t fixed4_load_aligned(const fixed_t* p)        { return vld1q_s32(p); }
static inline void      fixed4_store(fixed_t* p, fixed_4_t v)        { vst1q_s32(p, v); }
static inline void      fixed4_store_aligned(fixed_t* p, fixed_4_t v){ vst1q_s32(p, v); }

static inline fixed_4_t fixed4_add_impl_(fixed_4_t a, fixed_4_t b) { return vaddq_s32(a, b); }
static inline fixed_4_t fixed4_sub_impl_(fixed_4_t a, fixed_4_t b) { return vsubq_s32(a, b); }
static inline fixed_4_t fixed4_neg_impl_(fixed_4_t a)              { return vnegq_s32(a); }

/* Widen int32x4 -> int64x2 lo/hi, multiply, logical-shift-right by FIXED_SHIFT, narrow back. */
static inline fixed_4_t fixed4_mul_impl_(fixed_4_t a, fixed_4_t b) {
    int64x2_t lo64 = vmull_s32(vget_low_s32(a),  vget_low_s32(b));   /* lanes 0,1 */
    int64x2_t hi64 = vmull_s32(vget_high_s32(a), vget_high_s32(b));  /* lanes 2,3 */
    ...
    return vreinterpretq_s32_u32(vcombine_u32(lo32, hi32));
}
...
/* Compare ops return a 4-bit mask: bit i set iff lane i true. */
static inline uint8_t fixed4_eq_impl_(fixed_4_t a, fixed_4_t b) { return fixed4_movemask_neon_(vceqq_s32(a, b)); }
static inline uint8_t fixed4_gt_impl_(fixed_4_t a, fixed_4_t b) { return fixed4_movemask_neon_(vcgtq_s32(a, b)); }
static inline uint8_t fixed4_lt_impl_(fixed_4_t a, fixed_4_t b) { return fixed4_movemask_neon_(vcltq_s32(a, b)); }
static inline uint8_t fixed4_ge_impl_(fixed_4_t a, fixed_4_t b) { return fixed4_movemask_neon_(vcgeq_s32(a, b)); }
static inline uint8_t fixed4_le_impl_(fixed_4_t a, fixed_4_t b) { return fixed4_movemask_neon_(vcleq_s32(a, b)); }

#else /* FIXED_BACKEND_SCALAR */

typedef struct { int32_t v[4]; } fixed_4_t;
...
static inline fixed_4_t fixed4_load(const fixed_t* p)           { fixed_4_t r; memcpy(r.v, p, 4 * sizeof(fixed_t)); return r; }
static inline fixed_4_t fixed4_load_aligned(const fixed_t* p)   { fixed_4_t r; memcpy(r.v, p, 4 * sizeof(fixed_t)); return r; }
static inline void      fixed4_store(fixed_t* p, fixed_4_t v)        { memcpy(p, v.v, 4 * sizeof(fixed_t)); }
static inline void      fixed4_store_aligned(fixed_t* p, fixed_4_t v){ memcpy(p, v.v, 4 * sizeof(fixed_t)); }

#define FIXED4_LANEWISE_(OP) do { fixed_4_t r; for (int i = 0; i < 4; ++i) r.v[i] = (OP); return r; } while (0)

static inline fixed_4_t fixed4_add_impl_(fixed_4_t a, fixed_4_t b) { FIXED4_LANEWISE_((fixed_t)((uint32_t)a.v[i] + (uint32_t)b.v[i])); }
static inline fixed_4_t fixed4_sub_impl_(fixed_4_t a, fixed_4_t b) { FIXED4_LANEWISE_((fixed_t)((uint32_t)a.v[i] - (uint32_t)b.v[i])); }
static inline fixed_4_t fixed4_neg_impl_(fixed_4_t a)              { FIXED4_LANEWISE_((fixed_t)(0u - (uint32_t)a.v[i])); }
static inline fixed_4_t fixed4_mul_impl_(fixed_4_t a, fixed_4_t b) { FIXED4_LANEWISE_(fixed_mul(a.v[i], b.v[i])); }
static inline fixed_4_t fixed4_div_impl_(fixed_4_t a, fixed_4_t b) { FIXED4_LANEWISE_(fixed_div(a.v[i], b.v[i])); }

#define FIXED4_MASK_(OP) do { uint8_t m = 0; for (int i = 0; i < 4; ++i) if (OP) m |= (uint8_t)(1u << i); return m; } while (0)

static inline uint8_t fixed4_eq_impl_(fixed_4_t a, fixed_4_t b)  { FIXED4_MASK_(a.v[i] == b.v[i]); }
static inline uint8_t fixed4_gt_impl_(fixed_4_t a, fixed_4_t b)  { FIXED4_MASK_(a.v[i] >  b.v[i]); }
static inline uint8_t fixed4_lt_impl_(fixed_4_t a, fixed_4_t b)  { FIXED4_MASK_(a.v[i] <  b.v[i]); }
static inline uint8_t fixed4_ge_impl_(fixed_4_t a, fixed_4_t b)  { FIXED4_MASK_(a.v[i] >= b.v[i]); }
static inline uint8_t fixed4_le_impl_(fixed_4_t a, fixed_4_t b)  { FIXED4_MASK_(a.v[i] <= b.v[i]); }
